...
function setupInfoGridCollapse() {
    document.querySelectorAll('.item-info-grid .info-couple .info-label .collapse-toggle').forEach(row => {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (grid && (hasInfoGrid || hasItemMarker || isItemURL)) {
        setTimeout(() => {
            ...
            highlightActiveItem(getCurrentFile());
        }, 10);
    }
});
...
function setupItemHoverHighlight() {
    document.querySelectorAll('.item').forEach(item => {
        ...
        item.addEventListener('mouseleave', () => {
            ...
            highlightActiveItem(currentHref);
        });
    });
}
